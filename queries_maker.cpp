 ...
 * *
 */ 
void serializeQueries(int n, int k_min, int k_max, string outPath){
	//Read structure from file
	d_s data = populateDataStructure(BIG_DATA);
	//Output file stream
	ofstream outfile;
	outfile.open(outPath,ios::out);
	
	//Get max and min date from dataset
	pair<int,int> dates = getMaxMinDate(SMALL_DATA);
	for(int i=0; i<n; i++){
...
int main(){
	...
	serializeQueries(10000,5,10,"test.txt");
}
